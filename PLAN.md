...
2. **Shared types (`private_multisig_core/`).** Mirror lez-multisig's split. PDA seeds: `b"pmsig_state__"`, `b"pmsig_prop___"`, `b"pmsig_vault__"` (14 bytes each, matching lez-multisig's XOR-with-create_key pattern — see lez-multisig's PDA derivation `AccountId = SHA256(program_id ‖ seed)`). `// VERIFY:` confirm against `lez-multisig/multisig_core/src/pda.rs` whether the convention is XOR-with-create_key or SHA-256-of-concat before locking these seeds in — the two are mutually exclusive and the comment above conflates them. Instructions:
...
4. **Verifier program (`methods/guest/src/bin/private_multisig.rs`).** SPEL `#[lez_program]` with five `#[instruction]` handlers — same dispatch shape as lez-multisig (`create_multisig`, `propose`, `approve`, `reject`, `execute`) but with the approval handler swapped for a Risc0-receipt-verifying variant. The `approve` handler:
